...
class BrowserApp {
    ...
    async init() {
        ...
        if (customStart && customStart !== 'default' && customStart.trim() !== '') {
    ...
    checkAILimit() {
        ...
        if (usage.count >= 15) return false;
        usage.count++;
    ...
    setupIPC() {
        ...
        window.electronAPI.onAIAction(async(data) => {
            ...
            if (!usageCount) {
                aiContent.innerHTML = `<div style="color:var(--danger); padding:20px;">🚨 Daily Limit erreicht! Komm morgen wieder (Max. 15 Anfragen/Tag).</div>`;
                return;
            }
            aiContent.innerHTML = `
                <div style="background:var(--bg-hover); padding:15px; border-radius:8px; margin-bottom:15px; border-left: 4px solid var(--accent);">
                    <strong>Du hast gefragt:</strong><br>"${escapeHTML(data.text)}"
                </div>
                <div id="ai-response" style="color:var(--text-main); line-height: 1.8;"><i class="ph ph-spinner ph-spin text-green"></i> OpenAI überlegt... (Anfrage ${usageCount}/15)</div>`;
            try {
                const response = await fetch('https://openrouter.ai/api/v1/chat/completions', {
                    method: 'POST',
                    headers: { 'Authorization': 'Bearer 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', 'Content-Type': 'application/json' },
                    body: JSON.stringify({ model: 'openai/gpt-3.5-turbo', messages: [{ role: 'user', content: `Erkläre mir diesen Text kurz auf Deutsch: ${data.text}` }] })
                });
                const result = await response.json();
                document.getElementById('ai-response').innerHTML = escapeHTML(result.choices[0].message.content).replace(/\n/g, '<br>');
            } catch (err) {
                document.getElementById('ai-response').innerHTML = `<span style="color:var(--danger);">Fehler bei der Verbindung zur AI.</span>`;
            }
        });

        window.electronAPI.onTabAction((data) => {
            if (data.action === 'duplicate') {
                const tab = this.tabManager.tabs.get(data.id);
                if (tab) this.tabManager.createTab(tab.url, tab.isPrivate);
            }
...
class TabManager {
    ...
    createTab(url = START_PAGE, isPrivate = false, lazy = false) {
        ...
        const tabData = { id, webview, tabEl, url, title: 'Neu', isPrivate, isSnoozed: lazy, lastActive: Date.now(), currentZoom: 1 };
    ...
    bindEvents(tabData) {
        ...
        // WICHTIG: Hier wurde async hinzugefügt für die Passwort-Logik
        webview.addEventListener('dom-ready', async() => {
            this.app.extensions.inject(webview);

            // --- NEU: PASSWORT MANAGER LOGIK (AUTO-FILL & CAPTURE) ---
            ...
            // --- ENDE PASSWORT MANAGER LOGIK ---

            // BUGFIX: Mausgesten-Injection mit ;null; geschützt
            webview.executeJavaScript(`
                let __startX = 0;
        ...
        // NEU: Empfängt das abgefangene Passwort aus der Webview (Logins)
        ...
        webview.addEventListener('context-menu', (e) => {
            window.electronAPI.showContextMenu({ selectionText: e.params.selectionText });
        });
...
class UIManager {
    ...
    // NEU: Zeigt den Passwort Speichern Dialog
    showPasswordPrompt(domain, username, password) {
        const prompt = document.getElementById('password-prompt');
        if (!prompt) return; // Sicherheits-Check falls HTML noch nicht da ist
    ...
    setupEventListeners() {
        ...
        document.getElementById('btn-back').onclick = () => this.getActiveWebview()?.goBack();
        document.getElementById('btn-forward').onclick = () => this.getActiveWebview()?.goForward();
        document.getElementById('btn-reload').onclick = () => this.getActiveWebview()?.reload();
        ...
        document.getElementById('menu-splitview').onclick = () => {
            this.app.tabManager.toggleSplitView();
            document.getElementById('main-menu').classList.remove('active');
        };
        ...
        findInput.addEventListener('input', (e) => {
            if (this.app.tabManager.activeTabId) window.electronAPI.findInPage({ tabId: this.app.tabManager.activeTabId, text: e.target.value });
        });
        document.getElementById('find-close').onclick = () => {
            findBar.classList.remove('active');
            if (this.app.tabManager.activeTabId) window.electronAPI.findInPage({ tabId: this.app.tabManager.activeTabId, text: '' });
        };
        ...
        window.addEventListener('keydown', (e) => {
            ...
            else if (combo === shorts.print) { e.preventDefault(); this.getActiveWebview()?.print(); }
            else if (combo === shorts.cmdPalette) { e.preventDefault(); this.app.cmdPalette.open(); }
            else if (e.key === 'F5' || combo === 'ctrl+r') { e.preventDefault(); e.shiftKey ? this.getActiveWebview()?.reloadIgnoringCache() : this.getActiveWebview()?.reload(); }
        });
    }

    handleZoom(delta) {
        const id = this.app.tabManager.activeTabId;
        if (!id) return;
        const tab = this.app.tabManager.tabs.get(id);
        if (tab) {
            tab.currentZoom += delta;
            tab.webview.setZoomLevel(tab.currentZoom);
            this.showToast(`🔍 Zoom: ${Math.round((tab.currentZoom + 1) * 100)}%`);
        }
    }

    getActiveWebview() {
        if (!this.app.tabManager) return null;
        const id = this.app.tabManager.activeTabId;
        return id ? this.app.tabManager.tabs.get(id).webview : null;
    }
    ...
    applyTheme(theme) {
        if (theme === 'auto') {
            ...
            document.body.className = isNight ? 'dark-mode' : 'light-mode';
        } else {
            document.body.className = theme === 'gaming' ? 'gaming-mode' : (theme === 'light' ? 'light-mode' : '');
        }
    }
    ...
    showToast(msg) {
        ...
        t.className = 'toast'; t.innerHTML = `<i class="ph ph-info"></i> ${escapeHTML(msg)}`;
    ...
    handleDownloadStart(data) {
        ...
        if (document.getElementById('downloads-modal').classList.contains('active')) this.renderDownloadsList();
    }
    ...
    renderDownloadsList() {
        ...
        this.app.downloads.forEach((dl, index) => {
            ...
            if (dl.state === 'completed') el.querySelector('.btn-open-file').addEventListener('click', (e) => window.electronAPI.openFile(e.currentTarget.getAttribute('data-path')));
            el.querySelector('.btn-open-folder').addEventListener('click', (e) => window.electronAPI.openFolder(e.currentTarget.getAttribute('data-path')));
            el.querySelector('.btn-remove-dl').addEventListener('click', (e) => { const idx = parseInt(e.currentTarget.getAttribute('data-index')); this.app.downloads.splice(idx, 1); localStorage.setItem('phil_downloads', JSON.stringify(this.app.downloads)); this.renderDownloadsList(); });
            list.appendChild(el);
        });
    }
}
